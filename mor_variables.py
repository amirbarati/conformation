...
def get_base():
  ...
  personal_base = "/Users/evan/vsp/b2ar_analysis"
  ...
  if os.path.exists(sherlock_base):
    ...
    base = biox3_base
  elif(os.path.exists(personal_base)):
    print("were on a laptop")
    base = personal_base
  else:
    print("WHERE ARE WE?")
    sys.exit()
...
base = "/cstor/stanford/pande/users/enf/md_simulations/MOR/mor_active_apo_crystalwaters/analysis"
if not os.path.exists(base): os.makedirs(base)
traj_dir = "/cstor/stanford/pande/users/enf/md_simulations/MOR/mor_active_apo_crystalwaters/reimaged"
...
ori_tica_dir = "%s/%stICA_t%d_n_components%d%s_regularization%s%s%s" %(base, is_sparse, lag_time, n_components, feature_types, wolf_string, shrinkage_string, rho_string)
tica_dir = ori_tica_dir
if ktica: tica_dir = "%s/ktICA_n_components%d_random_specified_regularization%s%s" %(tica_dir, k_tica_components, k_tica_wolf_string, k_tica_shrinkage_string)
