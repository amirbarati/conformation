...
is_active <- function(row) {
  #print(row)
  #print(rownames(row))
  if(row["tm6_tm3_dist"] > 12.0 & row["npxxy_rmsd_active"] < row["npxxy_rmsd_inactive"] & row["connector_rmsd_inactive"] > row["connector_rmsd_active"] & row["connector_rmsd_active"] < 0.5 & row["npxxy_rmsd_active"] < 0.5) {
    return(TRUE)
  } else {
    return(FALSE)
  }
...
docking_csv <- "/Users/evan/biox3/b2ar_analysis_sherlock_all/b2ar_analysis/tICA_t10_n_components5_switches_npxx_tm6_bp/analysis_n_clusters1000/aggregate_docking.csv"
